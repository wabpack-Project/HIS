$.extend({
	...
	inputTipAjaxBefore: function (val, obj, fn) {
		...
		// 请求参数
		var data = {"word": val, "limit": 20, "supplier_id": $.global.data.supplier_id, "department": $.global.data.department};
		fn(data, val);
	},
	// 输入联想成功方法
	...
	getChildVal: function(el) {
		...
		// 存储 科室id和供应商id
		$.global.data.supplier_id = $(el).attr("attr_supplier_id");	// 供应商id
		$.global.data.supplier_name = $.global.supplier_arr[$.global.data.supplier_id].name;	// 供应商名称
		$.global.data.department = $(el).attr("attr_department");	// 科室id
		$.global.supplier_id = $(el).attr("attr_supplier_id");	// 供应商id
		$.global.supplier_name = $.global.supplier_arr[$.global.supplier_id].name;	// 供应商名称
		// console.log($.global.supplier_id, $.global.supplier_name);
	...
	updateDrug: function(n, obj) {
		...
		$(drug_td[8]).text(obj.price_total||"");
	...
	getNewDrug: function() {
		...
		$.global.newDrug.price_total = $("#price_sell").val();
	...
	calculatedAmount: function() {
		// 正则替换非法字符
		// $(this).resetInput(/[^0-9]/ig);
		// 获取成本单价和数量
		var price_check = parseFloat($("#price_check").val()||0),
			count = parseInt($(this).resetInput(/[^0-9]/ig)||0);// 正则替换非法字符，并返回非法值
		// console.log("库存数", $.global.number_sur_buy);
		// 判断数量为0
		if (count == 0)$(this).val("");
		// 输入数量先验证新增药品数据是否存在
		if (!$.global.newDrug || !$.global.newDrug.input_id){
			alert("请选择一个药品（您的药品名称，需要从弹出列表里选择）!");
			$(this).val("");
		}
	...
	stockInputOpt: function() {
		...
		$printStockInput.click(function () {
			...
			// $.global.data.tit = "药品" + $.global.data.type_name + "单";
			// 判断是否有打印信息，提示并退出
			...
			// 当前页面直接打印
			$.printOpt(tpl.adjust_output_prt, $.global.data);
		});
		// 清空操作
		...
		// 绑定操作类型事件
		$get_type_arr.change(function () {
			$.setSupplierOpt(this, $("#get_supplier"));
		});
	...
	setAutoComplete: function(){
		...
		$.lkinputtipsOpt.ajaxParameter.url = $.ajaxConfig.reqUrl("stock_adjust/adjust:output/act:get_auto_input");
	...
	popStateOpt: function(flag, res) {
		...
		// 移除药品列表数据
		$("#drug_table .g_tbody").remove();
		// console.log(0, $.global.data.detail);
		// 判断是否存在备份数据
	...
	// 根据操作类型设置供应商操作
	setSupplierOpt: function(typeEl, supplierEl){
		var type_name = $(typeEl).find("option:selected").text(),
				is_disable = supplierEl[0].hasAttribute("disabled");
		...
		if (type_name != "退货出库"){
			...
			$.global.data.supplier_id = 0;
			$.global.data.supplier_name = "";
			// 判断供应商是否禁用，如果禁用清空供应商信息
			if (!is_disable){
				// 禁用供应商下拉
				$.disableSelect(supplierEl, $.global.data.supplier_name, $.global.data.supplier_id);
			} else {
				// 设置select值
				$(supplierEl).children(":first").text($.global.data.supplier_name);
				// 设置select的id
				$(supplierEl).children(":first").attr("attr_id", $.global.data.supplier_id);
			}
		} else {
			// $.global.data.supplier_id = $.global.supplier_id || 0;
			// $.global.data.supplier_name = $.global.supplier_name || "";
			// 判断已选药品且有供应商，不能更改供应商直接赋值
			if (!!$.global.supplier_name){
				// 设置select值
				$(supplierEl).children(":first").text($.global.supplier_name);
				// 设置select的id
				$(supplierEl).children(":first").attr("attr_id", $.global.supplier_id);
				return;
			}
			// 没选药品时，判断供应商是禁用状态重新启用
			if (is_disable){
				// 启用供应商下拉
				$.onSelectN(supplierEl);
			}
		}
		// console.log("supplier_id", $.global.data.supplier_id);
		// console.log("supplier_name", $.global.data.supplier_name);
	...
	// 根据操作类型设置供应商操作
	setSupplierOptBase: function(typeEl, supplierEl){
		var type_name = $(typeEl).find("option:selected").text(),
			is_disable = supplierEl[0].hasAttribute("disabled");
		// console.log(type_name);
		if (type_name == "退货出库" || type_name == ""){
			// 没选药品时，判断供应商是禁用状态重新启用
			if (is_disable){
				// 启用供应商下拉
				$.onSelectOne(supplierEl);
			}
		} else {
			// 判断供应商是否禁用，如果禁用清空供应商信息
			if (!is_disable){
				// 禁用供应商下拉
				$.disableSelect(supplierEl, "", 0);
			}
		}
	},
	...
	getStockInputData : function (flag) {
		...
		// 请求URL
		$.ajaxConfig.url = $.ajaxConfig.reqUrl("stock_adjust/adjust:output/act:edit");
		// 请求参数
		$.ajaxConfig.data = {
		...
		$.ajaxConfig.reqDataApi($.ajaxConfig,function (res) {
			if (res.res && res.res == 1) {
				...
				// console.log(flag);
				// 模板替换
				var _html = miniTpl(tpl.adjust_output, res);
				// console.log(_html);
	...
	saveStockInputData : function () {
		...
		// 请求URL
		$.ajaxConfig.url = $.ajaxConfig.reqUrl("stock_adjust/adjust:output/act:save");
	...
	auditStockInputData : function () {
		...
		// 请求URL
		$.ajaxConfig.url = $.ajaxConfig.reqUrl("stock_adjust/adjust:output/act:audit");
	...
	delStockInputData : function (id) {
		// 请求URL
		$.ajaxConfig.url = $.ajaxConfig.reqUrl("stock_adjust/adjust:output/act:del_output");
		// http://h.lk.cn/drugstore/tab:stock_adjust/adjust:output/act:del_output
		// 请求参数
		$.ajaxConfig.data = {
			id: id
		};
	...
	delStockInputList : function (id, statusId) {
		...
		// 请求URL
		$.ajaxConfig.url = $.ajaxConfig.reqUrl("stock_adjust/adjust:output/act:del_document");
	...
	onInit: function(){
		...
		$("#type").change(function () {
			$.setSupplierOptBase(this, $("#supplier_id"));
		});
