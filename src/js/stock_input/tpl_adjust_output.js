...
var tpl = {
	...
	drugData: '<%\n' +
		...
		'  var item = data[i-1];\n' +
		'  %>\n' +
		'  <tr class="auto_complates" attr_department="<%=item.department||\'\'%>" attr_supplier_id="<%=item.supplier_id||\'\'%>"  attr_drug_id="<%=item.drug_id||\'\'%>"  attr_id="<%=item.id||\'\'%>" data-name="<%=item.goods_name||\'\'%>">\n' +
	...
	popbar: '<div class="pop_drug_head">\n' +
		'  <span class="pop_drug_tit">编辑调节出库</span>\n' +
	...
	drugInfo : '<%\n' +
		...
		'</tr>\n' +
		'<% } } %>',
	adjust_output : '<div id="stock_output" class="h_c_regist h_drugstore" >\n' +
		'  <!-- 入库表单 -->\n' +
		...
		'  </div>\n' +
		'</div>',
	adjust_output_prt: '<div class="prt">\n' +
		'    <%\n' +
		'    var printInfo = data,\n' +
		...
		'      <div class="col_md">出库科室：<%=department_name||""%></div>\n' +
		'      <div class="col_md fr">验收单号：<%=document_no||""%></div>\n' +
		'    </div>\n' +
		'    <!-- 表格 -->\n' +
		'    <div class="prt_table" >\n' +
		...
		'      <div class="col_md">出库人员：<%=verify_name||""%></div>\n' +
		'      <div class="col_md">出库时间：<%=verify_date||""%></div>\n' +
		'    </div>\n' +
		'  </div>',
};