var tpl = {
	...
	popbar: '<div class="pop_drug_head">\n' +
		'  <span class="pop_drug_tit">编辑验收入库</span>\n' +
	...
	drugInfo : '<%\n' +
		...
		'    <% } } %>',
	drugInfo1 : '<%\n' +
		'var d, drugInfo = data[data.length-1], _num = data.length-1;\n' +
		' %>\n' +
		'<tr class="g_tbody" id="drug_<%=_num%>" >\n' +
		'  <td><%=drugInfo.drug_name||""%></td>\n' +
		'  <td><%=drugInfo.drug_spec||""%></td>\n' +
		'  <td><%=drugInfo.batch_no||""%></td>\n' +
		'  <td><%=drugInfo.product_date||""%></td>\n' +
		...
		'  <td><%=drugInfo.price_sell||""%></td>\n' +
		'  <td><%=drugInfo.drug_factory||""%></td>\n' +
		'  <td class="drug_opt"><a attr_id="<%=drugInfo.drug_id%>" attr_num="<%=_num%>" href="javaScript:;" onclick="$.editDrug(this, <%=_num%>)">编辑</a>|<a attr_id="<%=drugInfo.drug_id%>" attr_num="<%=_num%>" href="javaScript:;" onclick="$.delDrug(this, <%=_num%>)">删除</a></td>\n' +
		'</tr>',
	stock_input : '<div id="stock_input" class="h_c_regist h_drugstore" >\n' +
		'  <!-- 入库表单 -->\n' +
		'  <div class="h_regist_head" >\n' +
		'      <div class="stock_input clearf" >\n' +
		'        <div class="form_group fl"> <span>验收科室：</span>\n' +
		...
		'              <%}else{%>\n' +
		'              <option attr_id="<%=supplierData[m].id%>"  attr_py_code_lower="<%=supplierData[m].py_code&&supplierData[m].py_code.toLowerCase()%>"  attr_py_code="<%=supplierData[m].py_code%>" value="<%=supplierData[m].id%>"><%=supplierData[m].name||\'\'%></option>\n' +
		'              <%}%>\n' +
		'  \n' +
		'              <% } %>\n' +
		'            </select>\n' +
		...
		'          <input id="batch_storage" class="input_btn btn_b36 radius3" type="submit" value="批量入库">\n' +
		'        </div>\n' +
		'      </div>\n' +
		'  </div>\n' +
		'  <!-- 添加药品 -->\n' +
